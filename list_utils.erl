...
%%
%% @doc Miscellaneous utilities related to lists and ASTs representing types.
...
%% @doc It successively applies a function F on an initial value and collects the
...
%% @doc Similar to {@link unfoldr}, but the resulting list is reversed.
...
%% @doc It converts an integer into its base 10 representation as a string.
...
%% @doc It pretty-prints the type passed as parameter.
...
%% @doc It pretty-prints the given type specification.
-spec spec_to_string (ast()) -> string().    
spec_to_string(Spec) -> lists:droplast(lists:flatten(erl_pp:form(Spec))).

    
%% @doc It performs a pre-post-order traversal of the AST corresponding to a type
%%
%% This is done as the `erl_syntax_lib' module (see 
%% [http://erlang.org/doc/man/erl_syntax_lib.html]) cannot handle this kind of
%% ASTs.
%%
...
fold_type(FPre, FPost, St, {type, _, field_type, [_, Type]} = T) ->
    FPost(T, fold_type(FPre, FPost, FPre(T, St), Type));
fold_type(FPre, FPost, St, 
            {type, _, binary, [{integer,_,0}, {integer,_,0}]} = T) ->
    FPost(T, FPre(T, St));
...
fold_type(FPre, FPost, St, {type, _, binary, [Type1, Type2]} = T) ->
    FPost(T, 
        fold_type(FPre, FPost, 
            fold_type(FPre, FPost, FPre(T, St), Type1),
            Type2));
fold_type(FPre, FPost, St, 
            {type, _, 'fun', [{type, _, product, []}, Type]} = T) ->
    FPost(T, fold_type(FPre, FPost, FPre(T, St), Type));
...
fold_type_list(FPre, FPost, St, Types) ->
    lists:foldl(fun(T, S) -> fold_type(FPre, FPost, S, T) end,
                St, Types).
            

%% @doc It replaces the children of the type given as first parameter with
